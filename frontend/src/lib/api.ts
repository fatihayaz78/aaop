...
export async function apiPatch<T>(path: string, body: unknown, tenantId: string = "bein_sports"): Promise<T> {
  const res = await fetch(`${BASE_URL}${path}`, {
    method: "PATCH",
    headers: headers(tenantId),
    body: JSON.stringify(body),
...
export async function apiDelete(path: string, tenantId: string = "bein_sports"): Promise<void> {
  const res = await fetch(`${BASE_URL}${path}`, {
    method: "DELETE",
    headers: headers(tenantId),
  });
